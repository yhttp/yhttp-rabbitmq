...
from yhttp.ext.rabbitmq import connpool
...
def consume(params):
    ...
    channel = cxn.channel()
    channel.queue_declare(queue='pika_pool_test')
    channel.basic_consume('pika_pool_test', _callback, auto_ack=False)
...
def test_invalidate_connection(queued_pool):
    Message.generate()
...
def test_timeout(empty_queued_pool):
    empty_queued_pool.timeout = 2
    st = time.time()
    with pytest.raises(connpool.Timeout):
        empty_queued_pool.acquire()
    elapsed = time.time() - st
    assert elapsed < 2.5


def test_timeout_override(empty_queued_pool):
    st = time.time()
    with pytest.raises(connpool.Timeout):
