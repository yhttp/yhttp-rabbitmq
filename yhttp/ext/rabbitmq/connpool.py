...
class Connection(object):
    ...
    def is_connection_invalidated(cls, exc):
        """Determine to invalidate connection.
...
class Pool(object):
    ...
    class Fairy(object):
        ...
        def cxn_str(self):
            ...
            if params:
                return '{0}:{1}/{2}'.format(params.host, params.port,
                                            params.virtual_host)

        def __str__(self):
            return ', '.join('{0}={1}'.format(k, v) for k, v in [
                ('cxn', self.cxn_str),
                ('channel', '{0}'.format(int(self.channel)
                 if self.channel is not None else self.channel)),
            ])
...
class QueuedPool(Pool):
    ...
    def acquire(self, timeout=None):
        ...
        if self.is_expired(fairy):
            print('closing expired connection - %s' % fairy)
            self.close(fairy)
            return self.acquire(timeout=timeout)
        if self.is_stale(fairy):
            print('closing stale connection - %s' % fairy)
            self.close(fairy)
            return self.acquire(timeout=timeout)
